...
<html lang="id" class="h-full">
 ...
 <body class="h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 text-gray-800 font-sans flex flex-col">
  ...
  <script>
        ...
        let selectedMonthFilter = (new Date().getMonth() + 1).toString(); 
        ...
        function readLegacyLocalStorage() {
            ...
            const recordsRaw = localStorage.getItem('vmi_app_data');
            const usersRaw = localStorage.getItem('vmi_users');
            const logsRaw = localStorage.getItem('vmi_activity_logs');
            if (!recordsRaw && !usersRaw && !logsRaw) {
                return null;
            }
        ...
        async function importLocalStorageData() {
            const payload = readLegacyLocalStorage();
            if (!payload) {
                alert('Tidak ada data lama di localStorage.');
                return;
            }

            if (!confirm('Import data lama dari localStorage ke database?')) {
                return;
            }
            ...
            if (!res.ok) {
                ...
                alert(error.message || 'Gagal import data lama.');
                return;
            }

            const result = await res.json();
            localStorage.removeItem('vmi_app_data');
            localStorage.removeItem('vmi_users');
            localStorage.removeItem('vmi_activity_logs');
            ...
            alert(`Import selesai. Data: ${counts.records || 0}, User: ${counts.users || 0}, Log: ${counts.logs || 0}.`);
        }
            ...
            apiRequest('/api/logs', {
                ...
                .then(payload => {
                    if (payload && payload.data) {
                        activityLogs.unshift(payload.data);
        ...
        window.dataSdk = {
            ...
            create: async (data) => {
                ...
                if (!res.ok) {
                    ...
                    return { isOk: false, message: payload.message || 'Gagal menyimpan data' };
                }
                const payload = await res.json();
                if (payload.data) {
                ...
                return { isOk: true, data: payload.data };
            },
            update: async (backendId, updatedFields) => {
                const res = await apiRequest(`/api/data/${encodeURIComponent(backendId)}`, {
                    method: 'PUT',
                    body: updatedFields
                });
                if (!res.ok) {
                    const payload = await res.json().catch(() => ({}));
                    return { isOk: false, message: payload.message || 'Gagal memperbarui data' };
                }
                const payload = await res.json();
                const idx = currentData.findIndex(d => d.__backendId === backendId);
                if (idx !== -1 && payload.data) {
                    const oldData = currentData[idx];
                    currentData[idx] = payload.data;
            ...
            delete: async (record) => {
                const res = await apiRequest(`/api/data/${encodeURIComponent(record.__backendId)}`, {
                    method: 'DELETE'
                });
                if (!res.ok) {
                    const payload = await res.json().catch(() => ({}));
                    return { isOk: false, message: payload.message || 'Gagal menghapus data' };
                }
        ...
        async function handleLogin() {
            ...
            if (!res.ok) {
                ...
                if (res.status === 419) {
                    message = 'CSRF token mismatch. Refresh halaman lalu coba lagi.';
                } else {
                    try {
                        const payload = await res.json();
                        message = payload.message || payload.errors?.username?.[0] || message;
                    } catch (error) {
                        message = `Login gagal (HTTP ${res.status}).`;
                    }
                }
                alert(message);
                return;
                ...
                <div class="bg-white rounded-lg p-6 shadow-xl border border-blue-200">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex items-center gap-2">
                            <button onclick="importLocalStorageData()" class="bg-amber-500 text-white px-4 py-2 rounded font-semibold">Import Data Lama</button>
                            ...
                            <tbody>
                                ${userDatabase.map((u, idx) => `
                                    <tr class="border-b">
                                        ...
                                        <td class="p-3 uppercase text-xs font-bold ${u.role==='admin'||u.role==='super admin'?'text-red-600':'text-green-600'}">${u.role}</td>
                                        <td class="p-3 text-center">
                                            <div class="flex flex-row justify-center gap-3">
                                                ${(currentUser.role === 'admin' && (u.role === 'super admin' || u.id === currentUser.id)) 
                                                    ? '<span class="text-gray-400 text-[10px] font-bold">TERKUNCI</span>' 
                                                    : `<button onclick="showEditUserForm('${u.id}')" class="text-blue-600 font-bold hover:underline uppercase text-[11px]">EDIT</button>`
                                                }
                                                ${(u.id !== '1' && u.role !== 'super admin' && u.id !== currentUser.id && !(currentUser.role === 'admin' && u.role === 'super admin')) ? `<button onclick="deleteUser('${u.id}')" class="text-red-600 font-bold hover:underline uppercase text-[11px]">HAPUS</button>` : (u.id === '1' || u.role === 'super admin' || u.id === currentUser.id ? '-' : '')}
                                            </div>
                                        </td>
                                    </tr>`).join('')}
                            </tbody>
        ...
        function renderUserForm(editData = null) {
            ...
            // LOGIKA: Admin tidak boleh edit detail Super Admin atau dirinya sendiri di modul ini
            const isRestricted = currentUser.role === 'admin' && editData && (editData.role === 'super admin' || editData.id === currentUser.id);
            ...
            content.innerHTML = `
                <div class="bg-white p-6 rounded-lg shadow-xl max-md mx-auto border border-blue-200">
                    ...
                    ${isRestricted ? '<p class="text-red-500 text-[10px] mb-4 font-black uppercase tracking-widest">Akses Terbatas: Admin tidak dapat mengubah data Super Admin atau datanya sendiri di sini.</p>' : ''}
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="n-name" value="${isEdit ? editData.name : ''}" ${isRestricted ? 'disabled' : ''} placeholder="Nama Lengkap" class="w-full p-2 border rounded">
                        </div>
                        <div>
                            <label class="text-xs font-bold text-gray-500 uppercase">Username</label>
                            <input type="text" id="n-user" value="${isEdit ? editData.username : ''}" ${isRestricted ? 'disabled' : ''} placeholder="Username" class="w-full p-2 border rounded">
                        </div>
                        <div>
                            <label class="text-xs font-bold text-gray-500 uppercase">Password</label>
                            <input type="password" id="n-pass" value="" ${isRestricted ? 'disabled' : ''} placeholder="${isEdit ? 'Biarkan kosong jika tidak diubah' : 'Password'}" class="w-full p-2 border rounded">
                        </div>
                        <div>
                            <label class="text-xs font-bold text-gray-500 uppercase">Role</label>
                            <select id="n-role" ${isRestricted ? 'disabled' : ''} class="w-full p-2 border rounded">
                                <option value="user" ${isEdit && editData.role === 'user' ? 'selected' : ''}>User</option>
                                ${currentUser.role === 'super admin' ? `<option value="admin" ${isEdit && editData.role === 'admin' ? 'selected' : ''}>Admin</option>` : ''}
                                ${currentUser.role === 'super admin' ? `<option value="super admin" ${isEdit && editData.role === 'super admin' ? 'selected' : ''}>Super Admin</option>` : ''}
                            </select>
                        </div>
                        <div class="flex gap-2">
                            ${!isRestricted ? `<button onclick="saveUser()" class="bg-blue-600 text-white px-4 py-2 rounded flex-1 font-bold">SIMPAN</button>` : ''}
        ...
        async function saveUser() {
            ...
            if (id) {
                ...
                if (idx !== -1) {
                    // PROTEKSI SISI LOGIKA: Admin dilarang edit Super Admin atau datanya sendiri
                    if (currentUser.role === 'admin' && (userDatabase[idx].role === 'super admin' || userDatabase[idx].id === currentUser.id)) {
                        alert('Gagal: Otoritas tidak mencukupi untuk mengubah data ini!');
                        return;
                    }
                }
                ...
                if (data.user) {
                    if (idx !== -1) {
                        userDatabase[idx] = data.user;
                    } else {
                        userDatabase.push(data.user);
                    }
                }
                saveLog(`Mengubah detail user: ${name}`);
            } else {
                // PROTEKSI: Admin tidak boleh buat Super Admin atau Admin
                if (currentUser.role === 'admin' && (role === 'super admin' || role === 'admin')) {
                    alert('Gagal: Admin hanya diizinkan membuat role User!');
                    return;
                }
                if (!password) {
        ...
        async function deleteUser(id) {
            ...
            // PROTEKSI: Admin dilarang hapus Super Admin atau dirinya sendiri
            if (currentUser.role === 'admin' && (u.role === 'super admin' || u.id === currentUser.id)) {
                alert('Akses Ditolak: Otoritas tidak mencukupi untuk menghapus akun ini!');
                return;
            }
                                ...
                                ${activityLogs.length === 0 ? '<tr><td colspan="7" class="p-10 text-center text-gray-400">Belum ada catatan aktivitas</td></tr>' : 
                                  activityLogs.map((log, idx) => `
                                    <tr class="border-b hover:bg-blue-50">
                                        ...
                                        <td class="p-3 uppercase text-[10px] font-bold ${log.role==='admin'||log.role==='super admin'?'text-red-600':'text-green-600'}">${log.role}</td>
        ...
        function getAllProgramsList() {
            const listProgramKelas = [
                ...
                'SR Intermedite', 'Minat Seni',
                'Arsitektur Gold', 'Arsitektur Silver', 'Arsitektur Bronze', 'Intensif USM UNPAR', 'Minat Arsitektur',
                'SR Anak Jangka Panjang', 'SR Anak Jangka Pendek', 'SR SMP Jangka Panjang', 'SR SMP Jangka Pendek',
        ...
        function navigateTo(module, submodule = '') {
            ...
            if (module === 'crm-followup' || module === 'target-peraihan') {
                selectedYearFilter = (currentSystemYear < 2026 ? defaultStartYear : currentSystemYear.toString());
                selectedMonthFilter = (new Date().getMonth() + 1).toString();
            } else {
        ...
        function renderContent() {
            ...
            if ((currentModule === 'manajemen-user' || currentModule === 'log-activity') && (currentUser.role !== 'admin' && currentUser.role !== 'super admin')) {
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
                        ...
                        <div class="table-container" style="max-height: 500px; overflow-y: auto;">
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    ${leadsDataFiltered.length === 0 ? `<tr><td colspan="${isAdmin ? '10' : '9'}" class="p-10 text-center text-gray-400">Tidak ada data untuk ditampilkan.</td></tr>` : 
                                      leadsDataFiltered.map((lead, idx) => {
                                        ...
                                        <tr class="border-b border-slate-100 hover:bg-blue-50 transition-all">
                                            ...
                                            <td class="p-3 text-center font-mono text-[10px] font-black ${lead.tgl_followup && isOverdue(lead.tgl_followup) ? 'text-rose-600 bg-rose-50 rounded-lg' : 'text-slate-600'}">
                                                ${lead.tgl_followup ? formatTanggal(lead.tgl_followup) : '-'}
                                            </td>
                                            <td class="p-3 text-center font-mono text-[10px] text-slate-600">
                                                ${lead.status_crm === 'Deal' ? formatTanggal(lead.tanggal_closing || lead.tanggal_input || lead.createdAt) : '-'}
                                            </td>
                                            <td class="p-3 text-[10px] italic text-slate-500 allow-wrap leading-relaxed">${lead.catatan_crm || '-'}</td>
                                            ${isAdmin ? `
                                            <td class="p-3 text-center">
                                                <div class="flex flex-row justify-center gap-3">
                                                    <button onclick="showUpdateCRMForm('${lead.__backendId}')" class="text-blue-600 font-bold hover:underline uppercase text-[11px]">EDIT</button>
                                                    <button onclick="deleteData('${lead.__backendId}')" class="text-red-600 font-bold hover:underline uppercase text-[11px]">HAPUS</button>
                                                </div>
                                            </td>` : ''}
                                        </tr>`}).join('')}
                                </tbody>
        ...
        function getCRMStatistics(data) {
            ...
            data.forEach(item => {
                ...
                const tglInputStr = item.tanggal_input || item.createdAt;
                if(tglInputStr) {
                    const dIn = new Date(tglInputStr);
                    const isYearMatch = selectedYearFilter === 'all' || dIn.getFullYear() == selectedYearFilter;
                    const isMonthMatch = selectedMonthFilter === 'all' || (dIn.getMonth() + 1) == selectedMonthFilter;
                    
                    if(isYearMatch && isMonthMatch) {
                        const dayIn = dIn.getDate();
                        if(dayIn >= 1 && dayIn <= daysInMonth) {
                            stats.daily.leads[dayIn-1]++; 
                        }
                    }
                }

                if(item.status_crm === 'Deal' || item.is_closing === true) {
                    const tglClosingStr = item.tanggal_closing || item.tanggal_input || item.createdAt;
                    const dCl = new Date(tglClosingStr);
                    const isYearMatchCl = selectedYearFilter === 'all' || dCl.getFullYear() == selectedYearFilter;
                    const isMonthMatchCl = selectedMonthFilter === 'all' || (dCl.getMonth() + 1) == selectedMonthFilter;

                    if(isYearMatchCl && isMonthMatchCl) {
                        const dayCl = dCl.getDate();
                        if(dayCl >= 1 && dayCl <= daysInMonth) {
                            stats.daily.closing[dayCl-1]++;
                        }
                    }
                }
            });
                    ...
                    <div class="space-y-4">
                        ...
                        <div id="section-biaya-deal" class="hidden space-y-4 p-3 bg-green-50 rounded-lg border border-green-200">
                            <div class="text-left">
                                ...
                                <select id="crm-program-siswa" class="w-full p-2 border border-green-300 rounded outline-none focus:ring-2 focus:ring-blue-400">
                        ...
                        <div id="section-catatan-standard" class="space-y-4">
                            <div class="text-left">
                                <label id="lbl-tgl-crm" class="text-xs font-bold text-gray-500 uppercase">${lead.status_crm === 'Deal' ? 'TANGGAL DAFTAR' : 'TANGGAL FOLLOW UP'}</label>
                                <input type="date" id="crm-date" value="${lead.tgl_followup || lead.tanggal_closing || ''}" class="w-full p-2 border rounded border-blue-300 outline-none focus:ring-2 focus:ring-blue-400">
                            </div>
                            <div class="text-left">
        ...
        function toggleBiayaInput() {
            ...
            const labelTgl = document.getElementById('lbl-tgl-crm');
            
            if (status === 'Deal') {
                section.classList.remove('hidden');
                if (labelTgl) labelTgl.textContent = 'TANGGAL DAFTAR';
            } else {
                section.classList.add('hidden');
                if (labelTgl) labelTgl.textContent = 'TANGGAL FOLLOW UP';
            }
        }

        async function saveCRMProgress(backendId) {
            const status = document.getElementById('crm-status').value;
            const tglAction = document.getElementById('crm-date').value; 
            ...
            if (status === 'Deal') {
                const isAlreadyConverted = currentData.some(d => d.module === 'data-siswa' && d.fromLeadsId === backendId);
                
                if (!isAlreadyConverted) {
                    await window.dataSdk.update(backendId, {
                        status_crm: 'Deal',
                        tanggal_closing: tglAction, 
                        catatan_crm: catatan,
                    ...
                    const payloadSiswa = {
                        ...
                        createdAt: tglAction ? new Date(tglAction).toISOString() : new Date().toISOString()
                    };
                    await window.dataSdk.create(payloadSiswa);
                    alert('Status CLOSING Berhasil!');
                } else {
                    alert('Data ini sudah pernah dikonversi menjadi siswa.');
                }
            } else {
                await window.dataSdk.update(backendId, {
                    status_crm: status,
                    tgl_followup: tglAction,
                    catatan_crm: catatan,
                    is_closing: false
                });
                alert('Progres Follow Up diperbarui!');
            }
            renderCRMFollowUp();
        }
        ...
        async function deleteData(backendId) {
            if (currentUser.role !== 'admin' && currentUser.role !== 'super admin') { alert('Hanya Admin!'); return; }
        ...
        function renderTargetPeraihanDetail() {
            ...
            if (selectedMonthFilter !== 'all') {
                filteredTargetData = filteredTargetData.filter(d => d.bulan == selectedMonthFilter);
                filteredSiswaData = filteredSiswaData.filter(d => (new Date(d.createdAt).getMonth() + 1) == selectedMonthFilter);
            }
            ...
            const showDailyChart = selectedMonthFilter !== 'all' && selectedYearFilter !== 'all';
            let dailySiswaStats = null;
            if (showDailyChart) {
                const daysInMonth = new Date(selectedYearFilter, selectedMonthFilter, 0).getDate();
                const dailyClosing = new Array(daysInMonth).fill(0);
                const dailyOmset = new Array(daysInMonth).fill(0);

                filteredSiswaData.forEach(siswa => {
                    const d = new Date(siswa.createdAt);
                    if ((d.getMonth() + 1) == selectedMonthFilter && d.getFullYear() == selectedYearFilter) {
                        const dayIdx = d.getDate() - 1;
                        dailyClosing[dayIdx]++;
                        
                        const biaya = parseFloat(siswa.biaya_pendidikan) || 0;
                        const sisa = parseFloat(siswa.sisa_angsuran) || 0;
                        dailyOmset[dayIdx] += (biaya - sisa);
                    }
                });
                ...
                <div class="bg-white rounded-lg p-6 shadow-xl border border-blue-200 font-['Poppins']">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <select id="month-filter-tm" onchange="applyTMFilter()" 
                                        class="px-3 py-2 rounded bg-white text-gray-800 border border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm font-['Poppins']">
                                    <option value="all" ${selectedMonthFilter === 'all' ? 'selected' : ''}>Semua Bulan</option>
                                    ${daftarNamaBulan.slice(1).map((m, idx) => `<option value="${idx+1}" ${selectedMonthFilter == idx+1 ? 'selected' : ''}>${m}</option>`).join('')}
                                </select>
                    ...
                    <div id="daily-chart-container" class="${showDailyChart ? '' : 'hidden'} bg-white p-5 rounded-2xl border border-blue-100 shadow-md mb-8">
                        <h3 class="text-xs font-black text-blue-700 mb-6 uppercase text-center tracking-widest">Grafik peraihan siswa dan Omset Harian</h3>
                        <div class="h-64"><canvas id="chart-tm-daily-sync"></canvas></div>
                    </div>
            ...
            setTimeout(() => { 
                ...
                if (showDailyChart && dailySiswaStats) {
                    renderDailySyncChart(dailySiswaStats);
                }
            }, 100);
        }

        function renderDailySyncChart(stats) {
            const ctx = document.getElementById('chart-tm-daily-sync');
            if (!ctx) return;
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: { 
                        legend: { position: 'top', labels: { font: { size: 10, family: 'Poppins', weight: 'bold' } } },
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                    ...
                    scales: {
                        y: { 
                            ...
                            position: 'left',
                            beginAtZero: true, 
                            title: { display: true, text: 'Jumlah Siswa', font: { size: 10, family: 'Poppins' } },
                        ...
                        y1: {
                            ...
                            position: 'right',
                            beginAtZero: true,
                            title: { display: true, text: 'Omset (Rupiah)', font: { size: 10, family: 'Poppins' } },
        ...
        function generateTargetPeraihanChartData(filteredTargetData, programFilter, currentLoc) {
            ...
            const aggregation = {};
            let totalTargetSiswa = 0, totalRealisasiSiswa = 0, totalTargetOmset = 0, totalRealisasiOmset = 0;
            ...
            const allSiswa = currentData.filter(d => d.module === 'data-siswa' && d.location === currentLoc);

            Object.keys(aggregation).forEach(key => {
                const data = aggregation[key];
                
                const realSiswaList = allSiswa.filter(s => {
                    if (programFilter !== 'all' && s.program !== programFilter) return false;
                    const dSiswa = new Date(s.createdAt);
                    return (dSiswa.getMonth() + 1) == data.bulan && dSiswa.getFullYear() == data.tahun;
                });

                const uniqueSiswa = Array.from(new Map(realSiswaList.map(s => [s.nama.toLowerCase(), s])).values());

                data.realSiswa = uniqueSiswa.length;
                data.realOmset = uniqueSiswa.reduce((acc, s) => {
                    const biaya = parseFloat(s.biaya_pendidikan || 0);
                    const sisa = parseFloat(s.sisa_angsuran || 0);
                    return acc + (biaya - sisa);
                }, 0);
            });
        ...
        function applyTMFilter() {
            ...
            selectedMonthFilter = document.getElementById('month-filter-tm')?.value || 'all';
            renderTargetPeraihanDetail();
        }
        ...
        function renderTargetPeraihanForm(location, editData = null) {
            ...
            document.getElementById('form-target-market').addEventListener('submit', async (e) => {
                ...
                if (result.isOk) {
                    navigateTo('target-peraihan', location);
                } else {
                    alert(result.message || 'Gagal menyimpan target.');
                }
            });
        ...
        function renderDataLeadsDetail() {
            const location = currentSubModule || 'Bandung';
            let filteredData = currentData.filter(d => d.module === 'data-leads' && d.wilayah === location);
            
            if (selectedStatusFilter === 'not_closing') {
                filteredData = filteredData.filter(d => !d.is_closing);
            } else if (selectedStatusFilter === 'is_closing') {
                filteredData = filteredData.filter(d => d.is_closing);
            }
            ...
            content.innerHTML = `
                <div class="bg-white rounded-lg p-6 shadow-xl border border-blue-200 font-['Poppins']">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <div class="flex items-center gap-2">
                                <select id="status-filter-leads" onchange="applyLeadsFilter()" 
                                        class="px-3 py-2 rounded bg-white text-gray-800 border border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm font-['Poppins']">
                                    <option value="all" ${selectedStatusFilter === 'all' ? 'selected' : ''}>Semua Status</option>
                                    <option value="not_closing" ${selectedStatusFilter === 'not_closing' ? 'selected' : ''}>Belum Closing</option>
                                    <option value="is_closing" ${selectedStatusFilter === 'is_closing' ? 'selected' : ''}>Sudah Closing</option>
                                </select>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                <tbody id="tbody-leads">
                                    ${filteredData.length === 0 ? `
                                        ...
                                        ` : filteredData.map((lead, idx) => {
                                            ...
                                            const isClosingStatus = lead.is_closing || lead.status_crm === 'Deal';
                                            return `
                                        <tr class="border-b border-blue-100 hover:bg-blue-50 text-gray-700 transition-colors ${isClosingStatus ? 'bg-emerald-50' : ''}">
                                            <td class="p-2 text-center text-[10px] font-mono tracking-tighter">${idx + 1}</td>
                                            <td class="p-2 text-left text-[10px] font-bold ${isClosingStatus ? 'text-emerald-700' : 'text-black'} uppercase tracking-tight" title="${lead.nama_siswa}">${lead.nama_siswa} ${isClosingStatus ? '✓' : ''}</td>
        ...
        function applyLeadsFilter() {
            selectedStatusFilter = document.getElementById('status-filter-leads')?.value || 'all'; 
        ...
        function renderLeadsForm(location, editData = null) {
                    ...
                    <form id="form-data-leads" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
                            ...
                            <div class="text-left">
                                ...
                                <input type="text" name="alamat" value="${isEdit ? editData.alamat : ''}" required class="w-full p-2 border rounded border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400 font-['Poppins']">
                            </div>
                        </div>
            ...
            document.getElementById('form-data-leads').addEventListener('submit', async (e) => {
                ...
                if (result.isOk) {
                    navigateTo('data-leads', location);
                } else {
                    alert(result.message || 'Gagal menyimpan data leads.');
                }
            });
        }
