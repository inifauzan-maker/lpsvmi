...
<html lang="id" class="h-full">
 ...
 <body class="h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 text-gray-800 font-sans flex flex-col">
  ...
  <script>
        ...
        function mapDataSiswa(item) {
            return {
                ...item,
                module: 'data-siswa',
                __backendId: item.id?.toString(),
                createdAt: item.tanggal_daftar || item.created_at
            };
        }

        function mapDataLeads(item) {
            return {
                ...item,
                module: 'data-leads',
                __backendId: item.id?.toString(),
                createdAt: item.tanggal_input || item.created_at
            };
        }

        function mapTargetMarket(item) {
            return {
                ...item,
        ...
        async function refreshAllData() {
            const [siswa, leads, targetMarket, logs, users] = await Promise.all([
                apiRequest('/data-siswa'),
                apiRequest('/data-leads'),
                apiRequest('/target-market'),
                apiRequest('/log-aktivitas'),
                apiRequest('/users')
            ]);

            currentData = [
                ...(siswa || []).map(mapDataSiswa),
                ...(leads || []).map(mapDataLeads),
                ...(targetMarket || []).map(mapTargetMarket)
            ];
            activityLogs = (logs || []).map(mapLogActivity);
            userDatabase = users || [];
        ...
        async function saveLog(action) {
            ...
            try {
                const log = await apiRequest('/log-aktivitas', {
                    method: 'POST',
                    body: JSON.stringify(payload)
                });
                if (log) {
                    activityLogs.unshift(mapLogActivity(log));
        ...
        function mapPayloadForModule(module, payload) {
            const data = { ...payload };
            delete data.module;
            ...
            if (module === 'data-siswa') {
                if (data.createdAt) {
                    data.tanggal_daftar = new Date(data.createdAt).toISOString().split('T')[0];
            ...
            if (module === 'data-leads') {
                if (data.createdAt) {
                    data.tanggal_input = new Date(data.createdAt).toISOString().split('T')[0];
                    delete data.createdAt;
                }
                return data;
            }
        ...
        function getEndpointForModule(module) {
            if (module === 'data-siswa') return '/data-siswa';
            if (module === 'data-leads') return '/data-leads';
            if (module === 'target-market') return '/target-market';
            return null;
        }
        ...
        window.dataSdk = {
            ...
            create: async (data) => {
                if (!canManageData()) {
                    alert('Akses ditolak. Hanya Admin atau Super Admin yang dapat menambah data.');
                    return { isOk: false };
                }
                const module = data.module;
                const endpoint = getEndpointForModule(module);
                if (!endpoint) return { isOk: false };

                const payload = mapPayloadForModule(module, data);
                ...
                try {
                    ...
                    await saveLog(`Menambah data pada modul ${module || 'Unknown'}`);
                    await refreshAllData();
                    return { isOk: true };
                } catch (error) {
                    alert(error.message);
            ...
            update: async (backendId, updatedFields) => {
                if (!canManageData()) {
                    alert('Akses ditolak. Hanya Admin atau Super Admin yang dapat mengubah data.');
                    return { isOk: false };
                }
                const record = currentData.find(d => d.__backendId === backendId);
                if (!record) return { isOk: false };

                const endpoint = getEndpointForModule(record.module);
                if (!endpoint) return { isOk: false };

                const payload = mapPayloadForModule(record.module, updatedFields);

                try {
                    await apiRequest(`${endpoint}/${backendId}`, {
                        method: 'PUT',
                        body: JSON.stringify(payload)
                    });
                    await saveLog(`Mengubah data pada modul ${record.module || 'Unknown'}`);
                    await refreshAllData();
                    return { isOk: true };
                } catch (error) {
                    alert(error.message);
            ...
            delete: async (record) => {
                if (!canManageData()) {
                    alert('Akses ditolak. Hanya Admin atau Super Admin yang dapat menghapus data.');
                    return { isOk: false };
                }
                const endpoint = getEndpointForModule(record.module);
                if (!endpoint) return { isOk: false };
                ...
                try {
                    await apiRequest(`${endpoint}/${record.__backendId}`, { method: 'DELETE' });
                    await saveLog(`Menghapus data pada modul ${record.module || 'Unknown'}`);
                    await refreshAllData();
                    return { isOk: true };
                } catch (error) {
                    alert(error.message);
        ...
        async function handleLogin() {
            ...
            try {
                ...
                await saveLog('Berhasil Login ke Sistem');
                location.reload();
            } catch (error) {
        ...
        async function handleLogout() {
            if (confirm('Logout dari sistem?')) {
                await saveLog('Keluar dari Sistem (Logout)');
                try {
                                    ...
                                    <tr class="border-b">
                                        ...
                                        <td class="p-3 text-center">
                                            <div class="flex flex-row justify-center gap-3">
                                                ...
                                                ${u.id !== currentUser.id ? `<button onclick="deleteUser('${u.id}')" class="text-red-600 font-bold hover:underline uppercase text-[11px]">HAPUS</button>` : '-'}
                                            </div>
                                        </td>
                                    </tr>`).join('')}
                            </tbody>
        ...
        function showEditUserForm(id) {
            const user = userDatabase.find(u => u.id.toString() === id);
                ...
                <div class="bg-white p-6 rounded-lg shadow-xl max-md mx-auto border border-blue-200">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="password" id="n-pass" value="" placeholder="${isEdit ? 'Biarkan kosong jika tidak diubah' : 'Password'}" class="w-full p-2 border rounded">
                        </div>
                        <div>
        ...
        async function saveUser() {
            ...
            try {
                if (id) {
                    ...
                    await saveLog(`Mengubah detail user: ${name}`);
                } else {
                    if (!password) {
                    ...
                    await saveLog(`Menambah user baru: ${name}`);
                }
                await refreshAllData();
                renderManajemenUser();
            } catch (error) {
        ...
        async function deleteUser(id) {
            const u = userDatabase.find(x => x.id.toString() === id);
            if (confirm('Hapus user ini?')) {
                try {
                    await apiRequest(`/users/${id}`, { method: 'DELETE' });
                    await saveLog(`Menghapus user: ${u ? u.name : 'Unknown'}`);
                    await refreshAllData();
                    renderManajemenUser();
                } catch (error) {
                    alert(error.message);
                                ...
                                ${activityLogs.length === 0 ? '<tr><td colspan="7" class="p-10 text-center text-gray-400">Belum ada catatan aktivitas</td></tr>' : 
                                  activityLogs.map((log, idx) => `
                                    <tr class="border-b hover:bg-blue-50">
                                        ...
                                        <td class="p-3 text-center">${formatTanggalPendek(log.date)}</td>
        ...
        async function clearLogs() {
            if (confirm('Hapus semua catatan aktivitas?')) {
                ...
                try {
                    await Promise.all(activityLogs.map(log => apiRequest(`/log-aktivitas/${log.id}`, { method: 'DELETE' })));
                    activityLogs = [];
                    renderLogActivity();
                } catch (error) {
                    alert(error.message);
        ...
        function renderContent() {
            ...
            if ((currentModule === 'manajemen-user' || currentModule === 'log-activity') && !hasFullAccess()) {
        ...
        async function deleteData(backendId) {
            if (!canManageData()) { alert('Hanya Admin atau Super Admin!'); return; }
            const record = currentData.find(d => d.__backendId === backendId);
            if (!record) return;
        ...
        function renderTargetPeraihanDetail() {
            ...
            // LOGIKA KONDISI GRAFIK HARIAN (SINKRONISASI DATA SISWA & OMSET)
        ...
        function generateTargetPeraihanChartData(filteredTargetData, programFilter, currentLoc) {
            ...
            filteredTargetData.forEach(item => {
                ...
                const realSiswaList = currentData.filter(s => {
                    if (s.module !== 'data-siswa' || s.location !== currentLoc) return false;
                    if (selectedProgramFilter !== 'all' && s.program !== selectedProgramFilter) return false;
